...
class PdBaseEnrichData(BaseEnrichData):
    """
    This is the base class for conducting enrichment on data.
    ...
    def __init__(self, module_name, version_name, rack_order, data_plus_meta, racks_map):
        """
        Constructor to initialize the enrichment module.
    ...
    def aggregate_over_days(self, df, cfg, agg_months):
        """
        ...
        Parameters:
            ...
            agg_months: number of months to look back for aggregation (i.e., 3)

        Returns:
            df: dataframe with one row per CTU for each customer.
        """

        agg_days = str(int(round(agg_months * cfg['TE_CONV_DICT'][cfg['agg_unit']]['day'],0))) + 'D'
...
#         df = self.create_rate_features(df, cfg, customer_interactions)
    ...
    def price_per_unit(self, df, cfg):
        ...
        df['price_per_unit'] = np.where(df[cfg['PRICE']] == 0, 0, df[cfg['PRICE']]/df[cfg['UNIT']])
    ...
    def payment_fail_ratio(self, df, cfg):
        ...
        print('creating payment fail ratio ...')
        df['payment_fail_ratio'] = np.where(df[cfg['TOTAL_PAYMENTS']] == 0, 0, df[cfg['FAILED_PAYMENTS']]/df[cfg['TOTAL_PAYMENTS']])
    ...
    def check_outliers(self, df, cfg, file_id):
        ...
        cfg['outlier_check_cols'].extend(cfg['TD_COLS'])
